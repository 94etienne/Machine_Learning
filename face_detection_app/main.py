...
while True:
    ...
    if faceCurFrame:
        ...
        for encodeFace, faceLoc in zip(encodeCurFrame,faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    counter =1
                    cvzone.putTextRect(imgBackground,"Loading",(90, 150)) #(90=X, 150 = Y)
            ...
            if counter!= 0:
                ...
                if counter == 1:
                    ...
                    # Set time limit to take attendance in 30 seconds. you set to 
                    # set time in seconds, if its hour convert to seconds, if is a day convert to seconds
                    datetimeObject = datetime.strptime(studentInfo['last_attendance_time'],
                    "%Y-%m-%d %H:%M:%S") #convert time from db to object
                    secondsElapsed = (datetime.now()-datetimeObject).total_seconds() #current time - db time
                    print(secondsElapsed)

                    # Set time to take attendance will take it once then take it again after 30 seconds
                    #  in production you can updated it to one day but converted in seconds
                    ...
                    # If attended it should displated below message
                    else:
                        modeType = 3 #(Image contain action of student found in resources/modes folder)
                        counter = 0
                        mode_img_resized_imgModeList = cv2.resize(imgModeList[modeType], (153, 232)) 
                        imgBackground[104:104+232, 650:650+153] = mode_img_resized_imgModeList  # Overlay mode image 
    ...
    # When student attended and want to anted again without exceeding 30 seconds
    else:
        modeType = 0
        counter = 0
